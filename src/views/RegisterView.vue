  ...
  try {
    ...
    switch (authError.code) {
      ...
      case 'auth/weak-password':
        registrationError.value = 'Password is too weak. It must be at least 8 characters long.';
        break;
      default:
  ...
  <AuthLayout title="Create Fakaloan Account">
    ...
    <Form :validation-schema="schema" @submit="registerUser" class="space-y-4">
      <div>
        ...
        <Field id="name" name="name" type="text" autocomplete="name" v-slot="{ field, errors }" :validate-on-input="true">
          <input v-bind="field" :class="['form-input-base', errors.length ? 'form-input-invalid' : 'form-input-valid']" placeholder="Thabo Mokoena" aria-describedby="name-error" />
        </Field>
      ...
      <div>
        ...
        <Field id="password" name="password" type="password" autocomplete="new-password" v-slot="{ field, errors }" :validate-on-input="true">
          <input v-bind="field" type="password" :class="['form-input-base', errors.length ? 'form-input-invalid' : 'form-input-valid']" placeholder="Password (min. 8 characters)" aria-describedby="password-error" />
        </Field>
        <ErrorMessage name="password" id="password-error" class="form-error-text" />
      </div>
      <div>
        <label for="passwordConfirmation" class="form-label">Confirm Password</label>
        <Field id="passwordConfirmation" name="passwordConfirmation" type="password" autocomplete="new-password" v-slot="{ field, errors }" :validate-on-input="true">
          <input v-bind="field" type="password" :class="['form-input-base', errors.length ? 'form-input-invalid' : 'form-input-valid']" placeholder="Confirm Password" aria-describedby="passwordConfirmation-error" />
        </Field>
        <ErrorMessage name="passwordConfirmation" id="passwordConfirmation-error" class="form-error-text" />
      </div>
      <div>
